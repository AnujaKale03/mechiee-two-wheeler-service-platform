...
import {
  ...
  Modal, Animated, Dimensions, Image, StatusBar,
} from "react-native";
...
const MENU_ITEMS = [
  {
    section: null,
    items: [
      { key: "bookings",      icon: "receipt-outline",         label: "My Bookings",       badge: null   },
      { key: "vehicles",      icon: "bicycle-outline",         label: "My Vehicles",       badge: null   },
      { key: "payments",      icon: "wallet-outline",          label: "Payment History",   badge: null   },
    ],
  },
  {
    section: "SUPPORT",
    items: [
      ...
      { key: "support",       icon: "headset-outline",         label: "Help & Support",    badge: null   },
  ...
  },
  {
    section: "ACCOUNT",
    items: [
      { key: "logout",        icon: "log-out-outline",         label: "Sign Out",          badge: null, danger: true },
    ],
  },
];
...
export default function ProfileDrawer({ visible, onClose, navigation }) {
  ...
  const loadProfile = async () => {
    try {
      const raw = await AsyncStorage.getItem("customerSession");
  ...
  const handleItem = async (key) => {
    onClose();
    setTimeout(() => {
      if (key === "bookings")     navigation.navigate("Bookings");
      if (key === "logout")       handleLogout();
      // Other keys can navigate to future screens
    }, 200);
  };
...
const styles = StyleSheet.create({
  ...
  closeBtn: {
    position: "absolute", top: 16, right: SPACING.md,
  ...
  profileName: {
    fontSize: FONT_SIZE.xl, ...FONTS.bold,
  ...
  sectionLabel: {
    fontSize: FONT_SIZE.xs, ...FONTS.bold,
  ...
  menuLabel: {
    ...
    ...FONTS.medium, color: COLORS.textPrimary,
  },
  menuBadge: {
  ...
  version: {
    ...
    ...FONTS.regular, color: COLORS.textDisabled,
