...
import { mechanicLogin } from "../../services/authService";
import { saveMechanicPushToken } from "../../utils/notifications";
...
export default function MechanicLoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      const res = await mechanicLogin(name.trim(), pin);
      await AsyncStorage.setItem("authToken", res.data.token);
      await AsyncStorage.setItem("authUser",  JSON.stringify(res.data.mechanic));
      await AsyncStorage.setItem("authRole",  "mechanic");
      await saveMechanicPushToken();
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.container}>
        <View style={styles.hero}>
          ...
          <Text style={styles.heroTitle}>Mechanic Portal</Text>
          <Text style={styles.heroSub}>Sign in to access your assignments</Text>
        </View>
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Sign In</Text>
          {error ? <View style={styles.errorBox}><Text style={styles.errorText}>⚠️  {error}</Text></View> : null}
          <Text style={styles.label}>Your Name</Text>
          <TextInput style={styles.input} placeholder="e.g. Mechanic A" placeholderTextColor={COLORS.textMuted} value={name} onChangeText={setName} autoCapitalize="words" />
          <Text style={styles.label}>4-Digit PIN</Text>
          <TextInput style={styles.input} placeholder="••••" placeholderTextColor={COLORS.textMuted} value={pin} onChangeText={setPin} keyboardType="numeric" maxLength={4} secureTextEntry />
...
const styles = StyleSheet.create({
  ...
  hero: { backgroundColor: COLORS.mechanicAccentPastel, alignItems: "center", paddingTop: 60, paddingBottom: SPACING.xxl, borderBottomLeftRadius: 36, borderBottomRightRadius: 36 },
  ...
  heroTitle: { fontSize: 22, ...FONTS.extraBold, color: COLORS.textOnMechanic, marginTop: SPACING.sm },
  heroSub: { fontSize: 13, ...FONTS.regular, color: COLORS.textOnMechanicMuted, marginTop: 4 },
  ...
  loginBtn: { marginTop: SPACING.lg, backgroundColor: COLORS.mechanicAccent, paddingVertical: SPACING.md, borderRadius: RADIUS.lg, alignItems: "center", ...SHADOW.md },
