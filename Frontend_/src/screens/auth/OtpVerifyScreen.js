...
import { verifyOtp as apiVerifyOtp } from "../../utils/authApi";
...
export default function OtpVerifyScreen({ navigation, route }) {
  ...
  async function handleVerify() {
    ...
    setError('');
    try {
      const data = await apiVerifyOtp(phone, code, role);
      const { token, user } = data;

      // Save token under the key api.js reads ("authToken")
      await AsyncStorage.setItem('authToken', token);
      await AsyncStorage.setItem('authRole', role);

      // Save profile so HomeScreen, ProfileDrawer, getBookings can read it
  ...
  return (
    <View style={styles.root}>
      ...
      <Text style={styles.subtitle}>
        OTP sent to <Text style={styles.phoneHighlight}>{maskedPhone}</Text>
      </Text>
      ...
      <View style={styles.resendRow}>
        {canResend ? (
          ...
          <Text style={styles.resendCooldown}>
            Resend in <Text style={{ color: '#888' }}>0:{String(countdown).padStart(2, '0')}</Text>
          </Text>
        )}
      </View>

      <TouchableOpacity
        style={[styles.btn, { backgroundColor: color }, (otp.join('').length < OTP_LENGTH || loading) && styles.btnDisabled]}
...
const styles = StyleSheet.create({
  root:           { flex: 1, backgroundColor: '#0F0F0F', paddingHorizontal: 24, paddingTop: 60 },
  backBtn:        { marginBottom: 32 },
  backArrow:      { fontSize: 22, color: '#888' },
  iconWrap:       { width: 64, height: 64, borderRadius: 16, borderWidth: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: '#1A1A1A', marginBottom: 24 },
  ...
  otpRow:         { flexDirection: 'row', gap: 12, marginBottom: 12, justifyContent: 'center' },
  otpBox:         { width: 64, height: 68, backgroundColor: '#1A1A1A', borderRadius: 12, borderWidth: 1.5, borderColor: '#2A2A2A', color: '#FFFFFF', fontSize: 26, fontWeight: '700' },
  otpFilled:      { backgroundColor: '#1A1A1A' },
  otpError:       { borderColor: '#F43F5E' },
  errorText:      { color: '#F43F5E', fontSize: 13, textAlign: 'center', marginBottom: 16 },
  ...
  btn:            { height: 52, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginBottom: 16 },
